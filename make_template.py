...
def main() -> None:
    ...
    if "arxiv" in args.url:
        ...
        paper_data.update({
            "title": str(html.select_one("h1").string),
            "authors": "[]",
...
def extract_arxiv_data(url: str) -> Dict[str, str]:
    ...
    title_matcher = Matcher("#abs > div.leftcolumn > h1",
                        mapping=lambda element, page: Raschietto.element_to_text(element)[6:])
    authors_matcher = Matcher("#abs > div.leftcolumn > div.authors",
                        mapping=lambda element, page: Raschietto.element_to_text(element)[8:].split(", "))
    year_matcher = Matcher("#abs > div.leftcolumn > div.dateline",
                        mapping=lambda element, page: Raschietto.element_to_text(element)[16:])

    paper_data["title"] = title_matcher(page)
    paper_data["authors"] = authors_matcher(page)

    year = year_matcher(page)
    paper_data["year"] = year[year.find("20"):year.find("20") + 4]
